...
const AddEditQuestModal = (props: IAddEditBookingModal) => {
  const [
    structure, variants, isOpenDelete, config
  ] = useUnit([
    $bookingsStructureStore, $bookingsVariantsStore, $bookingIsOpen, $bookingsVariantsConfig
  ]);
  ...
  const options = structure["quest_code"]?.values.map(elem => ({
    label: elem.name,
    value: elem.code
  }))
  ...
  const watchQuest = watch("quest_code")

  useEffect(() => {
    if (props.isOpen && watchQuest) {
      (async () => {
        await bookingsVariantsFx({ booking: props.location, date: selectedDate.toLocaleDateString(), questCode: watchQuest.value});
      })()
    }
  }, [selectedDate, watchQuest]);
  ...
  useEffect(() => {
    const defaultType = options && options.find(elem => elem.value === props.editable?.quest_code)
    if (props.editable && !isMounted) {
      setValue("quest_code", options && options.length > 0 && defaultType ? defaultType : options && options?.length > 0 ? options[0] :  null)
      setIsMounted(true)
    }
      ...
      <Modal centered={true} style={{zIndex: isOpenDelete ? "1000": ""}} show={props.isOpen} onHide={props.onClose}>
        ...
        <Modal.Body className="px-4 pb-4 pt-0">
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Form.Group className="mb-3">
              <Form.Label>Квест</Form.Label>
              <Controller control={control}
                          {...register("quest_code", {
                            required: `Укажите квест`,
                          })}
                          render={({field}) => (
              ...
              />
              {errors && errors["quest_code"] ? (
                <Form.Control.Feedback type="invalid" className="d-block">
                  {errors["quest_code"]?.message?.toString()}
            ...
            <Form.Group className="mb-3">
              ...
              <Controller control={control}
                          {...register("count_people", {
                            min: {
                              value: watchQuest && config?.types?.[watchQuest.value]?.min_count ? config.types[watchQuest.value].min_count : 1,
                              message: `Максимальное значение — ${watchQuest && config?.types?.[watchQuest.value]?.min_count ? config.types[watchQuest.value].min_count : 10}`,

                            },
                            max: {
                              value: watchQuest && config?.types?.[watchQuest.value]?.max_count ? config.types[watchQuest.value].max_count : 10,
                              message: `Максимальное значение — ${watchQuest && config?.types?.[watchQuest.value]?.max_count ? config.types[watchQuest.value].max_count : 10}`,
                            },
                            required: structure.required ? `Укажите количество людей` : false,
                          })}
                          render={({field}) => (
                            <Form.Control type="number"
                                          {...field}
                                          max={watchQuest && config?.types?.[watchQuest.value]?.max_count ? config?.types[watchQuest.value].max_count : 10}
                                          min={watchQuest && config?.types?.[watchQuest.value]?.min_count ? config?.types[watchQuest.value].min_count : 1}
                            />
                          )}
              />
